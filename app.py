...
def fetch_data():
    ...
    # Usando el ID garantizado para asegurar la conexión
    params = {
...
try:
    ...
    # Limpieza básica: quitamos la columna '_id' que es solo un correlativo interno
    ...
    # Buscamos la primera columna de texto para usarla como filtro principal
    columnas_texto = df.columns.tolist()
    columna_filtro = columnas_texto[0] 
    
    opciones = df[columna_filtro].unique().tolist()
    seleccion = st.sidebar.multiselect(f"Selecciona {columna_filtro}", opciones, default=opciones[:5]) # Seleccionamos 5 por defecto para no saturar
    ...
    # Contamos la frecuencia de los datos para graficar
